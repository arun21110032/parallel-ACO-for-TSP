...
void build_solution_thread(int start, int end);
void build_solution();
...
int main(int argc, char *argv[]) {
    ...
    cout << "\n runtime (ACO): " << calculate_time(time_start, clock()) << " ms" << endl;
    //cin.get();
    return 0;
}
...
void seed_initial_pheromone(bool random) {
    for (int i = 0; i < CITY_AMOUNT; i++) {
        for (int j = 0; j < CITY_AMOUNT; j++) {
            if (i != j) {
                double random_pheromone = (double)get_random_number(0, 100) / 100.0;
                pheromone_links[i][j] = (random == true) ? random_pheromone : PHEROMONE_RATE;
            }
            else {
        ...
        while (ants[i].position < (CITY_AMOUNT - 1)) {
            ...
            for (int j = 0; j < CITY_AMOUNT; j++) {
                if (check_visit(i, j) == false) {
                    if (pheromone_links[position][j] >= 0 && distance_links[position][j] >= 0) {
                        link_rate_sum += pow(pheromone_links[position][j], ALFA) * pow(greater_distance - distance_links[position][j], BETA);
                    }
                }
            }
            for (int j = 0; j < CITY_AMOUNT; j++) {
                if (check_visit(i, j) == false) {
                    if (pheromone_links[position][j] >= 0 && distance_links[position][j] >= 0) {
                        transition_probability[j] = (pow(pheromone_links[position][j], ALFA) * pow(greater_distance - distance_links[position][j], BETA)) / link_rate_sum;
                    }
                    else {
...
void build_solution() {
    ...
    for (int i = 0; i < num_threads; i++) {
        ...
        int end = (i == num_threads - 1) ? POPULATION_SIZE : start + chunk_size;
        threads.emplace_back(build_solution_thread, start, end);
    }
    ...
    for (int i = 0; i < CITY_AMOUNT; i++) {
        for (int j = 0; j < CITY_AMOUNT; j++) {
            if (pheromone_links[i][j] != INVALID) {
                pheromone_links[i][j] = (1 - EVAPORATION_RATE) * pheromone_links[i][j];
            }
        }
    }
}
